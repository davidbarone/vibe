...
#### Purpose
Checks that the key columns for a csv file contain unique values for the entire file. This is done by checking that the file data row count matches the number of distinct values of the key column(s). The special metadata column '__key' is used.
